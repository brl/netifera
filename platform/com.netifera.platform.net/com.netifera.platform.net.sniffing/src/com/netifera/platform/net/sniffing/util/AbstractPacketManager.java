...
public abstract class AbstractPacketManager<T extends IPacketHeader>
	...
	private void deliver(Set<IPacketSnifferHandle<T>> handleSet, T packet, IPacketContext ctx) {
		...
		for(IPacketSnifferHandle<T> handle : handleSet) {
			...
			deliverToHandle(handle, packet, ctx);
		}
	}

	private void deliverToHandle(IPacketSnifferHandle<T> handle, T packet, IPacketContext ctx) {
		final Object contextTag = ctx.getPacketTag();
		final Object handleTag = handle.getDefaultTag();

		if(contextTag == null && handleTag != null) {
			ctx.setPacketTag(handleTag);
			handle.getSniffer().handlePacket(packet, ctx);
			ctx.setPacketTag(null);
		} else {
			handle.getSniffer().handlePacket(packet, ctx);
		}
	}
